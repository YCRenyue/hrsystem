 ...
 * );
 */
exports.checkDataScope = (resourceType = 'employee') => {
  return async (req, res, next) => {
    try {
      const user = req.user;
 ...
 * );
 */
exports.requireDepartmentAccess = (deptIdParam = 'departmentId') => {
  return async (req, res, next) => {
    try {
      const user = req.user;
 ...
 * );
 */
exports.checkSensitiveDataAccess = (employeeIdParam = 'id') => {
  return async (req, res, next) => {
    try {
      const user = req.user;
 ...
 * );
 */
exports.requirePermissions = (...middlewares) => {
  return async (req, res, next) => {
    let index = 0;
 ...
 * @returns {Function} Express middleware
 */
exports.checkEditableFields = (employeeIdParam = 'id', requestedFields = []) => {
  return async (req, res, next) => {
    try {
      const user = req.user;
