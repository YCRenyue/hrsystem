...
- [x] 实现员工状态流转逻辑 (已完成 - 2025-12-05)
  - 实现状态机模式（EmployeeStatusService）
  - 状态转换规则：pending -> active -> inactive
  - 自动设置试用期结束日期和离职日期
  - 测试覆盖率：100%（11个测试用例全部通过）
...
- [x] 实现权限验证逻辑 (已完成 - 2025-12-05)
  - 权限常量定义完善（permissions.js）
  - 细粒度权限中间件（checkPermission, checkDataScope等）
  - User模型权限验证方法完善
  - 支持角色、数据范围、敏感数据访问控制
...
**估时**：3-4天 | **优先级**: 高 | **依赖**: 钉钉集成、数据模型 | **完成度**: 70%
...
- [x] 实现入职流程自动创建 (已完成 - 2025-12-05)
  - 创建OnboardingService服务
  - 创建onboarding_process记录
  - 生成form_token和form_link
  - 状态设为pending
  - 测试覆盖率：100%（6个测试用例全部通过）
...
- [x] 实现表单动态链接生成 (已完成 - 2025-12-05)
  - 唯一token生成（crypto.randomBytes）
  - URL拼接（generateFormLink方法）
  - OnboardingProcess模型支持

- [x] 实现表单访问权限控制 (已完成 - 2025-12-05)
  - Token验证（_findProcessByToken方法）
  - 防止重复提交（isCompleted检查）
  - 在OnboardingService中实现

- [x] 实现表单有效期管理 (已完成 - 2025-12-05)
  - 默认7天有效（token_expires_at）
  - 过期检查（isTokenExpired方法）
  - 自动标记过期（markAsExpired方法）

- [x] 实现表单提交数据合并 (已完成 - 2025-12-05)
  - 合并预录入数据（submitForm方法）
  - 更新员工记录（_updateEmployeeData方法）
  - 支持加密字段自动处理
  - 状态更新为completed
