...
>
> **最新更新**:
> - (2026-04-23) 食堂卡表导入：复用考勤卡表布局，将每日上午/下午打卡解析为午餐/晚餐消费记录；食堂页面改为食堂报表，提供每日/每周中餐晚餐人数、每人用餐汇总与明细查看
> - (2026-04-23) 考勤卡表导入：支持解析多工作表（1.3.4/5.6.7 等）按姓名匹配员工，存储上午/下午/加班打卡明细与周期汇总，新增考勤报表页（/attendance/report）展示迟到/早退/请假人员与次数
> - (2026-04-01) 制度确认模块升级：培训服务承诺函改为正式法律版本，支持HR配置费用/年限，模板变量自动填充；管理规章制度改为分页在线阅读（9页），需阅读全部后方可签署；新增 training_pledges 数据库表
> - (2026-03-01) 新增员工自助制度确认入口，侧边栏根据角色导航到不同页面
> - (2026-03-01) 新增公司制度阅读确认表和员工培训承诺函功能（制度确认模块）
...
> - (2025-12-22) 修复Dashboard控制器中入职趋势图表的字段名错误（hired_at -> entry_date）
> - (2025-12-04) 新增考勤管理和请假管理功能模块
> - (2025-12-04) 增强Dashboard页面，添加考勤率和请假统计
> - (2025-11-22) 完成前端UI增强和文档整理，所有核心功能已完成并通过全面测试（87个测试全部通过）

##  项目进度概览

- **整体完成度**: 78%
- **当前阶段**: 阶段十三 - 数据整合Dashboard增强
- **下一里程碑**: 完成数据库ID结构优化，测试所有Dashboard图表功能
...
- [ ] 入职流程自动化功能（30%） 进行中
- [x] HR管理界面（80%） 已完善
- [x] 制度确认模块（100%） 已完成 - 2026-04-01
  - [x] 培训服务承诺函升级为正式法律版本，含变量占位符自动填充 (2026-04-01)
    - HR可在管理界面配置每员工的培训费用和服务年限（仅2个字段）
    - 新建 training_pledges 数据库表存储费用/年限，employee 关联 hasOne
    - 员工签署时模板变量（姓名/身份证/金额/年限/服务期）自动填入
    - 金额自动转换大写（如 5000 → 伍仟元整）
  - [x] 培训服务承诺函甲乙双方信息支持员工手动修改 (2026-04-13)
    - 甲方（公司名称/住所地）和乙方（姓名/性别/身份证/地址/合同日期）均预填充默认值
    - 默认值来源：甲方为公司固定信息，乙方从数据库员工记录读取
    - 员工可在签署前编辑任意字段，修改后实时同步至文档预览
  - [x] 管理规章制度改为9页分页在线阅读，末页才可签署 (2026-04-01)
    - 前8页为制度内容，第9页为签署确认页
    - 顶部阅读进度条，签署区域在读完全部页面后解锁
- [ ] 员工自助服务（0%）
...
**估时**：4-5天 | **实际用时**: 1天 | **优先级**: 中 | **完成度**: 100%
**目标**：实现年假、社保公积金、出差补助、就餐记录等业务数据的管理

#### 12.1 年假管理模块 已完成

**优先级**: 高 | **依赖**: 员工管理、假期报表 | **完成日期**: 2025-12-05

- [x] 设计annual_leave表结构 (已完成 - 2025-12-05)
  - 字段：年假ID、员工ID、年度、应休天数、已休天数、剩余天数
  - 自动计算规则（根据入职时间）
  - 有效期管理

- [x] 实现年假API (已完成 - 2025-12-05)
  - GET /api/annual-leave - 获取年假列表（支持筛选）
  - GET /api/annual-leave/:id - 获取单条记录
  - POST /api/annual-leave - 创建年假记录
  - PUT /api/annual-leave/:id - 更新记录
  - DELETE /api/annual-leave/:id - 删除记录
  - GET /api/annual-leave/template - 下载Excel模板
  - POST /api/annual-leave/import - 导入Excel数据
  - GET /api/annual-leave/export - 导出Excel数据

- [x] 创建年假管理前端页面 (已完成 - 2025-12-05)
  - 文件：`frontend/src/pages/AnnualLeave/AnnualLeaveList.tsx`
  - 列表展示：员工编号、姓名、年度、应休、已休、剩余
  - 筛选：按年度、员工搜索
  - Excel导入导出功能（三按钮布局）
  - 剩余天数彩色标签

- [x] 创建年假添加/编辑表单页（已完成 - 2026-04-13）
  - 文件：`frontend/src/pages/AnnualLeave/AnnualLeaveForm.tsx`
  - 路由：`/annual-leave/new`、`/annual-leave/:id/edit`
  - 输入应休天数、已休天数、结转天数后自动实时计算剩余天数
  - 剩余天数以彩色数字展示（>5绿、>0橙、<=0红）
  - 列表页添加编辑按钮
...
- [x] 权限控制 (已完成 - 2025-12-05)
  - admin/hr_admin：查看所有、编辑、导入导出
  - department_manager：查看、导出
  - employee：查看个人年假

#### 12.2 社保公积金管理模块 已完成

**优先级**: 高 | **依赖**: 员工管理 | **完成日期**: 2025-12-05

- [x] 设计social_security表结构 (已完成 - 2025-12-05)
  - 字段：记录ID、员工ID、年月、社保基数、公积金基数
  - 缴纳金额：养老保险、医疗保险、失业保险、工伤保险、生育保险、公积金
  - 个人部分、公司部分（分开存储）
  - 缴纳状态：pending/paid/failed

- [x] 实现社保公积金API (已完成 - 2025-12-05)
  - GET /api/social-security - 获取记录列表
  - GET /api/social-security/:id - 获取单条记录
  - POST /api/social-security - 创建记录
  - PUT /api/social-security/:id - 更新记录
  - DELETE /api/social-security/:id - 删除记录
  - GET /api/social-security/template - 下载Excel模板
  - POST /api/social-security/import - 导入Excel数据
  - GET /api/social-security/export - 导出Excel数据

- [x] 创建社保公积金管理页面 (已完成 - 2025-12-05)
...
**优先级**: 低 | **依赖**: 员工管理、考勤管理 | **完成日期**: 2025-12-05

- [x] 设计canteen_meal表结构 (已完成 - 2025-12-05)
  - 字段：记录ID、员工ID、就餐日期、餐次（早/中/晚）
  - 就餐地点：公司食堂/外部餐厅
  - 餐费金额、支付方式
  - 是否补贴、补贴金额

- [x] 实现就餐记录API (已完成 - 2025-12-05)
  - GET /api/canteen-meals - 获取就餐记录
  - GET /api/canteen-meals/:id - 获取单条记录
  - POST /api/canteen-meals - 创建就餐记录
  - PUT /api/canteen-meals/:id - 更新记录
  - DELETE /api/canteen-meals/:id - 删除记录
  - GET /api/canteen-meals/template - 下载Excel模板
  - POST /api/canteen-meals/import - 导入Excel数据
  - GET /api/canteen-meals/export - 导出Excel数据

- [x] 创建就餐管理页面 (已完成 - 2025-12-05)
  - 文件：`frontend/src/pages/CanteenMeal/CanteenMealList.tsx`
  - 列表展示：员工信息、日期、餐次、地点、金额、补贴
  - 员工搜索功能
  - Excel导入导出功能（三按钮布局）
  - 补贴标签显示
...
### 版本 2.3.0 (2025-12-07)  最新
- **前端Bug修复与增强**：
  - 修复个性化设置功能问题
    - 修复颜色和字号选择后无法立即生效的问题，现在保存后立即应用
    - 修复ColorPicker选择框不显示当前默认值的问题
    - 优化个性化设置加载，页面加载时自动应用已保存的主题设置
  - 完善系统菜单导航
    - 在主菜单中添加请假管理入口
    - 在主菜单中添加年假管理入口
    - 在主菜单中添加社保管理入口
...
### 版本 2.3.0 (2026-04-23)
- **考勤卡表导入功能**（新增）
  - 后端新增 `POST /api/attendances/import-card` 接口
    - 使用 `xlsx` 包解析 .xls/.xlsx 文件，自动识别形如 `1.3.4`/`5.6.7` 等考勤卡表工作表
    - 每张卡表左/中/右排列 3 位员工，按每 15 列的偏移提取姓名、工号、日期范围与每日打卡
    - 根据员工姓名匹配花名册（考勤表工号与花名册工号不同，姓名加密后解密比对）
    - 未匹配员工、重名员工分别返回给前端人工核对
  - 后端新增 `GET /api/attendances/report` 接口
    - 按周期/部门聚合员工考勤汇总
    - 返回每人迟到/早退/请假/旷工/出差/加班统计及汇总合计
  - 数据模型
    - `attendances` 表新增 `morning_check_in/out`, `afternoon_check_in/out`, `overtime_check_in/out`, `source` 字段
    - 新增 `attendance_summaries` 表（迁移 `10-add-attendance-detail-fields.js`）
  - 前端增强
    - `AttendanceList` 页面显示上午/下午打卡列，新增「导入考勤卡表」按钮与「考勤报表」入口
    - 新增 `AttendanceReport` 页面（路由 `/attendance/report`）
      - 顶部 4 张统计卡片：迟到人员、早退人员、请假天数、加班总时长
      - TOP 榜：迟到 TOP / 早退 TOP / 请假 TOP
      - 汇总明细表格，可按列排序，支持按周期/部门筛选

### 版本 2.2.0 (2025-12-04)
- **后端新增功能**：
  - 创建考勤管理(Attendance)模块
    - 考勤记录CRUD API (`/api/attendances`)
    - 考勤统计API (`/api/attendances/stats`)
    - 支持按员工、日期、状态筛选
  - 创建请假管理(Leave)模块
    - 请假申请CRUD API (`/api/leaves`)
    - 请假统计API (`/api/leaves/stats`)
    - 支持多种假期类型和审批状态
  - 增强Dashboard API
    - 新增本月考勤率统计
    - 新增待审批请假数量
